...
$reponse = $bdd-> query('SELECT id_compte, pseudo, email, mdp, reponse FROM '.$table_compte.'');
$test1=false;
$test2=false;
$test3=false;
//$visit=0;
...
if($test1==true)
{
	if($test2==true)
	{
		...
		{
			header("Location: index1.php?error=Votre mot de passe est incorrect.");
			echo "Votre mot de passe est incorrect.";
		}
	}
	else
	{
		header("Location: index1.php?error=Votre mail ne correspond pas.");
		echo "Votre mail ne correspond pas.";
	}
}
else
{
	header("Location: index1.php?error=Votre pseudo est introuvable.");
